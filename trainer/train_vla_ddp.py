...
def init_trained_vlm(config: VLACofig, action_1st: List[float], action_99th: List[float], rank: int):
    # [DDP 修改] 只在主进程(rank 0)打印信息，避免日志混乱
    ...
    del vlm_state_dict, temp_vlm_model
...
def train_vla():
    ...
    for epoch in range(epochs):
        # [DDP 修改] 设置sampler的epoch，确保每个epoch的shuffle不同
        ...
        # [DDP 修改] 同步所有进程的验证指标
        metrics = torch.tensor([val_loss_sum, val_acc_sum, val_batches_sum, len(val_loader)]).to(device)
        dist.all_reduce(metrics, op=dist.ReduceOp.SUM)
        
        # [DDP 修改] 只在主进程计算并打印最终的平均指标
        if rank == 0:
            total_val_loss, total_val_acc, total_val_batches, total_val_loader_len = metrics.tolist()
